...
/*! \brief A RGB color representation. Each color component ranges between 0.0
 * and 1.0. */
...
public:
  ...
  //@{
  //! Default constructor. Default value is (0,0,0). Value range between 0.0 and
  //! 1.0.
  ...
  explicit Color(QRgb rgb)
      : r(qRed(rgb) / 255.0), g(qGreen(rgb) / 255.0), b(qBlue(rgb) / 255.0) {}
  ...
  //! Dereferencing operator that returns the memory address of the vector. You
  //! can hence use glColor3fv(myColor).
  operator const float *() const { return &r; };

  //! QRgb converter. Useful so that Color can be directly given as a parameter
  //! of QImage::setPixel().
  operator QRgb() const {
    return qRgb(qMin(int(255.0 * r), 255), qMin(int(255.0 * g), 255),
                qMin(int(255.0 * b), 255));
  };

  //! QString converter. Useful for debugging as in \c qWarning("Color is
  //! "+QString(color)) (Qt3).
  operator QString() const {
    return QString::number(int(255.0 * r)) + " " +
           QString::number(int(255.0 * g)) + " " +
           QString::number(int(255.0 * b));
  };
  //@}
  ...
  //! Returns the color divided by a scalar. Useful for Color averages.
  //! \attention Division by 0 is not checked.
  friend Color operator/(const Color &c, float k) {
    return Color(c.r / k, c.g / k, c.b / k);
  }
  ...
  //! Clamp each color component in [0.0..1.0]. Adding colors may lead to values
  //! higher than 1.0.
  void clamp() {
    r = (r < 1.0) ? r : 1.0;
    g = (g < 1.0) ? g : 1.0;
